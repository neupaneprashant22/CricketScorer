...
document.getElementById("toss").innerHTML+=toss+ " won the toss and decided to "+decide+" first.";
...
function add_batsman()
{
    ...
    td[4].textContent=Number(runs_scored[batsman_number])/Number(ball_faced[batsman_number]);
    ...
    noball.addEventListener("click",function(e)
    {
        ...
        out.addEventListener("click",function(e)
        {
            if(isstart!=true)
            {
                ...
                batsman[batsman_number]=prompt("Enter the name of bastman no."+batsman_number);
                add_batsman();
                showUpdated();
                checkStatus();
              }
...
function showUpdated()
{
    ...
    {
        // bowler_overs+=0.1;
        if(parseFloat(Math.abs(bowler_overs[current_bowler]-Math.floor(bowler_overs[current_bowler])).toFixed(1))==0.6)
        {
            bowler_overs[current_bowler]=Math.floor(bowler_overs[current_bowler])+1;
            checkMaiden();
        }
        overs=checkOvers(overs);
        overs=parseFloat(overs.toFixed(1));
        bowler_overs[current_bowler]=parseFloat(bowler_overs[current_bowler].toFixed(2));
    }
    ...
    document.getElementById("btsr"+onstrike).innerHTML=(Number((runs_scored[onstrike])/Number(ball_faced[onstrike]))*100).toFixed(2);

    //total areas's document manipulation
    document.getElementById("total-runs").innerHTML="Total runs: "+totalruns+"/"+totalwickets;
    document.getElementById("total-overs").innerHTML="Overs:"+overs;
    document.getElementById("total-runrate").innerHTML="Runrate:"+((totalruns/totalBalls)*6).toFixed(2);
    document.getElementById("to-win").innerHTML="Runs remaning:"+(target-totalruns);
    document.getElementById("balls-remaning").innerHTML="Balls remaning:"+((parseInt(myobj.overs*6))-totalBalls);
    document.getElementById("required-rr").innerHTML="Required Runrate:"+((target-totalruns)/((parseInt(myobj.overs*6))-totalBalls));

    //bowler document manipulation
    document.getElementById("bwwkt"+current_bowler).innerHTML=bowler_wickets[current_bowler];
    document.getElementById("bwmaiden"+current_bowler).innerHTML=bowler_maiden[current_bowler];
    document.getElementById("bwover"+current_bowler).innerHTML=bowler_overs[current_bowler];
    document.getElementById("bweco"+current_bowler).innerHTML=((bowler_runs[current_bowler])/(bowler_balls[current_bowler])*6).toFixed(2);

    if((parseFloat(overs).toFixed(1)==parseFloat(maxovers).toFixed(1))|| maxwickets==totalwickets)
    {
...
{
    var checkBowler=parseInt(prompt("Enter the S.N of the bowler(if new type 0)"));
    if(checkBowler==0)
    {
        bowler_number++;
